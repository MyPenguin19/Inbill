...
export default function ResultPage() {
  ...
  return (
    <main className="min-h-screen">
      <div className="mx-auto w-full max-w-7xl space-y-12 px-6 py-16 lg:px-12">
        {!PAYWALL_ENABLED ? (
          <div className="inline-flex rounded-xl border border-gray-100 bg-white px-3 py-2 text-xs font-medium text-gray-500 shadow-sm">
            Test Mode — Full report visible
          </div>
        ) : null}

        {error ? (
          <div className="bg-white rounded-2xl shadow-sm border border-red-200 p-6 text-sm font-medium text-red-700">
            {error}
          </div>
        ) : null}
        ...
        {report && analysisJson ? (
          <div className="space-y-12">
            <SummaryCard
              ...
              amount={overpaymentText}
              amountLabel="Estimated Overpayment"
              confidenceScore={confidenceScore}
              confidenceLabel={confidenceBarLabel}
              riskScore={riskScore}
              riskLabel={riskBarLabel}
              verdict={quickVerdict}
            />

            <IssueList issues={analysisJson.issues.map((issue) => ({
              title: normalizeIssueTitle(issue.title),
              description: normalizeIssueDescription(issue.title, issue.description),
              impact: `Estimated impact: ${issue.impact_range}`,
            }))} />
            ...
            <CallScriptCard
              sections={phoneScriptSections}
              copied={copiedScript === "phone"}
              onCopy={() => void handleCopyScript(phoneScriptSections.join("\n\n"), "phone")}
            />
            ...
            <section className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-4">
              <h2 className="text-lg font-semibold tracking-tight text-gray-950">Save and Share</h2>
            ...
            {!isPaid ? (
              <section className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-4">
                <h2 className="text-lg font-semibold tracking-tight text-gray-950">Unlock Full Report</h2>
                <p className="text-sm text-gray-700">
